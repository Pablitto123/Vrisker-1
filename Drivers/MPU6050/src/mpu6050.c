...
int MPU6050_getIntDataReadyStatus() {
    uint8_t buff;
    MPU6050_ReadBit(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_INT_STATUS, MPU6050_INTERRUPT_DATA_RDY_BIT, &buff);
    return buff;
}
...
uint8_t MPU6050_getIntDataReadyEnabled() {
    uint8_t buffer[14];
    MPU6050_ReadBit(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_INT_ENABLE, MPU6050_INTERRUPT_DATA_RDY_BIT, buffer);
    return buffer[0];
}
 ...
 * @see MPU6050_INTERRUPT_DATA_RDY_BIT
 */
void MPU6050_setIntDataReadyEnabled(uint8_t enabled) {
    MPU6050_WriteBit(MPU6050_DEFAULT_ADDRESS, MPU6050_RA_INT_ENABLE, MPU6050_INTERRUPT_DATA_RDY_BIT, enabled);
}
...
{
    while(HAL_I2C_Mem_Write(&hi2c1, slaveAddr, writeAddr, 1, pBuffer, 1,100)!=HAL_OK){
...
{
    while(HAL_I2C_Mem_Read(&hi2c1, slaveAddr, readAddr, 1, pBuffer, NumByteToRead,100)!=HAL_OK)
    {
        if (HAL_I2C_GetError(&hi2c1) != HAL_I2C_ERROR_AF) {
            Error_Handler();
        }
